...
void IFC4X3::IfcBuilding::getStepLine( std::stringstream& stream ) const
{
	...
	stream << ",";
	if( m_BuildingAddress ) { stream << "#" << m_BuildingAddress->m_tag; } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcBuilding::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcBuilding::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	const size_t num_args = args.size();
	if( num_args != 12 ){ std::stringstream err; err << "Wrong parameter count for entity IfcBuilding, expecting 12, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_ElevationOfRefHeight = IfcLengthMeasure::createObjectFromSTEP( args[9], map, errorStream );
	m_ElevationOfTerrain = IfcLengthMeasure::createObjectFromSTEP( args[10], map, errorStream );
	readEntityReference( args[11], m_BuildingAddress, map, errorStream );
}
void IFC4X3::IfcBuilding::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
...
void IFC4X3::IfcBuilding::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IFC4X3::IfcFacility::getAttributesInverse( vec_attributes_inverse );
}
void IFC4X3::IfcBuilding::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcFacility::setInverseCounterparts( ptr_self_entity );
}
void IFC4X3::IfcBuilding::unlinkFromInverseCounterparts()
{
