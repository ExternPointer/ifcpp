...
void IFC4X3::IfcConstructionProductResource::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConstructionProductResource, expecting 11, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcConstructionProductResourceTypeEnum::createObjectFromSTEP( args[10], map, errorStream );
}
