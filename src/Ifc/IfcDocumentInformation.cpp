...
void IFC4X3::IfcDocumentInformation::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 17 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDocumentInformation, expecting 17, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_Location = IfcURIReference::createObjectFromSTEP( args[3], map, errorStream );
	m_Purpose = IfcText::createObjectFromSTEP( args[4], map, errorStream );
	m_IntendedUse = IfcText::createObjectFromSTEP( args[5], map, errorStream );
	m_Scope = IfcText::createObjectFromSTEP( args[6], map, errorStream );
	m_Revision = IfcLabel::createObjectFromSTEP( args[7], map, errorStream );
	m_DocumentOwner = IfcActorSelect::createObjectFromSTEP( args[8], map, errorStream );
	readSelectList( args[9], m_Editors, map, errorStream );
	m_CreationTime = IfcDateTime::createObjectFromSTEP( args[10], map, errorStream );
	m_LastRevisionTime = IfcDateTime::createObjectFromSTEP( args[11], map, errorStream );
	m_ElectronicFormat = IfcIdentifier::createObjectFromSTEP( args[12], map, errorStream );
	m_ValidFrom = IfcDate::createObjectFromSTEP( args[13], map, errorStream );
	m_ValidUntil = IfcDate::createObjectFromSTEP( args[14], map, errorStream );
	m_Confidentiality = IfcDocumentConfidentialityEnum::createObjectFromSTEP( args[15], map, errorStream );
	m_Status = IfcDocumentStatusEnum::createObjectFromSTEP( args[16], map, errorStream );
}
