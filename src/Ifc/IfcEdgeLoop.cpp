...
void IFC4X3::IfcEdgeLoop::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_tag << "= IFCEDGELOOP" << "(";
	writeEntityList( stream, m_EdgeList );
	stream << ");";
}
void IFC4X3::IfcEdgeLoop::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcEdgeLoop::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 1 ){ std::stringstream err; err << "Wrong parameter count for entity IfcEdgeLoop, expecting 1, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
