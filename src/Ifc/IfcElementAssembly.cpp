...
IFC4X3::IfcElementAssembly::IfcElementAssembly( int tag ) { m_tag = tag; }
void IFC4X3::IfcElementAssembly::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_tag << "= IFCELEMENTASSEMBLY" << "(";
...
void IFC4X3::IfcElementAssembly::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcElementAssembly::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementAssembly, expecting 10, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_AssemblyPlace = IfcAssemblyPlaceEnum::createObjectFromSTEP( args[8], map, errorStream );
	m_PredefinedType = IfcElementAssemblyTypeEnum::createObjectFromSTEP( args[9], map, errorStream );
}
void IFC4X3::IfcElementAssembly::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IFC4X3::IfcElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "AssemblyPlace", m_AssemblyPlace ) );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IFC4X3::IfcElementAssembly::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IFC4X3::IfcElement::getAttributesInverse( vec_attributes_inverse );
}
void IFC4X3::IfcElementAssembly::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcElement::setInverseCounterparts( ptr_self_entity );
}
void IFC4X3::IfcElementAssembly::unlinkFromInverseCounterparts()
{
