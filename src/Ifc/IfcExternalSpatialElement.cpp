...
void IFC4X3::IfcExternalSpatialElement::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcExternalSpatialElement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcExternalSpatialElement::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcExternalSpatialElement, expecting 9, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcExternalSpatialElementTypeEnum::createObjectFromSTEP( args[8], map, errorStream );
}
