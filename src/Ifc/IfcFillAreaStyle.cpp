...
void IFC4X3::IfcFillAreaStyle::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 3 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFillAreaStyle, expecting 3, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_Name = IfcLabel::createObjectFromSTEP( args[0], map, errorStream );
	readSelectList( args[1], m_FillStyles, map, errorStream );
	m_ModelOrDraughting = IfcBoolean::createObjectFromSTEP( args[2], map, errorStream );
}
