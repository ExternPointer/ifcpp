...
IFC4X3::IfcGeographicElement::IfcGeographicElement( int tag ) { m_tag = tag; }
void IFC4X3::IfcGeographicElement::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_tag << "= IFCGEOGRAPHICELEMENT" << "(";
...
void IFC4X3::IfcGeographicElement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcGeographicElement::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcGeographicElement, expecting 9, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcGeographicElementTypeEnum::createObjectFromSTEP( args[8], map, errorStream );
}
void IFC4X3::IfcGeographicElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
	IFC4X3::IfcElement::getAttributes( vec_attributes );
	vec_attributes.emplace_back( std::make_pair( "PredefinedType", m_PredefinedType ) );
}
void IFC4X3::IfcGeographicElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IFC4X3::IfcElement::getAttributesInverse( vec_attributes_inverse );
}
void IFC4X3::IfcGeographicElement::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
	IfcElement::setInverseCounterparts( ptr_self_entity );
}
void IFC4X3::IfcGeographicElement::unlinkFromInverseCounterparts()
{
