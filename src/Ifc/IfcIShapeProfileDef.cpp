...
void IFC4X3::IfcIShapeProfileDef::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcIShapeProfileDef::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcIShapeProfileDef::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcIShapeProfileDef, expecting 10, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_OverallWidth = IfcPositiveLengthMeasure::createObjectFromSTEP( args[3], map, errorStream );
