...
void IFC4X3::IfcLibraryReference::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcLibraryReference::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcLibraryReference::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcLibraryReference, expecting 6, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_Language = IfcLanguageId::createObjectFromSTEP( args[4], map, errorStream );
	readEntityReference( args[5], m_ReferencedLibrary, map, errorStream );
}
