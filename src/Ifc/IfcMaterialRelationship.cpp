...
void IFC4X3::IfcMaterialRelationship::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcMaterialRelationship, expecting 5, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	readEntityReference( args[2], m_RelatingMaterial, map, errorStream );
	readEntityReferenceList( args[3], m_RelatedMaterials, map, errorStream );
	m_MaterialExpression = IfcLabel::createObjectFromSTEP( args[4], map, errorStream );
}
