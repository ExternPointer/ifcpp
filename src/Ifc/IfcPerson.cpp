...
void IFC4X3::IfcPerson::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPerson, expecting 8, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_Identification = IfcIdentifier::createObjectFromSTEP( args[0], map, errorStream );
	m_FamilyName = IfcLabel::createObjectFromSTEP( args[1], map, errorStream );
	m_GivenName = IfcLabel::createObjectFromSTEP( args[2], map, errorStream );
	...
	readEntityReferenceList( args[6], m_Roles, map, errorStream );
	readEntityReferenceList( args[7], m_Addresses, map, errorStream );
}
