...
void IFC4X3::IfcPointOnSurface::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 3 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPointOnSurface, expecting 3, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	readEntityReference( args[0], m_BasisSurface, map, errorStream );
	m_PointParameterU = IfcParameterValue::createObjectFromSTEP( args[1], map, errorStream );
	m_PointParameterV = IfcParameterValue::createObjectFromSTEP( args[2], map, errorStream );
}
