...
void IFC4X3::IfcPolynomialCurve::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 4 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPolynomialCurve, expecting 4, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	readEntityReference( args[0], m_Position, map, errorStream );
	readTypeOfRealList( args[1], m_CoefficientsX );
	readTypeOfRealList( args[2], m_CoefficientsY );
	readTypeOfRealList( args[3], m_CoefficientsZ );
}
