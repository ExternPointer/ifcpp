...
void IFC4X3::IfcPresentationLayerWithStyle::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPresentationLayerWithStyle, expecting 8, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_LayerOn = IfcLogical::createObjectFromSTEP( args[4], map, errorStream );
	m_LayerFrozen = IfcLogical::createObjectFromSTEP( args[5], map, errorStream );
	m_LayerBlocked = IfcLogical::createObjectFromSTEP( args[6], map, errorStream );
	readEntityReferenceList( args[7], m_LayerStyles, map, errorStream );
}
