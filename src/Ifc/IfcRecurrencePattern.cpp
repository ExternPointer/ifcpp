...
void IFC4X3::IfcRecurrencePattern::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRecurrencePattern, expecting 8, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_RecurrenceType = IfcRecurrenceTypeEnum::createObjectFromSTEP( args[0], map, errorStream );
	...
	m_Position = IfcInteger::createObjectFromSTEP( args[4], map, errorStream );
	m_Interval = IfcInteger::createObjectFromSTEP( args[5], map, errorStream );
	m_Occurrences = IfcInteger::createObjectFromSTEP( args[6], map, errorStream );
	readEntityReferenceList( args[7], m_TimePeriods, map, errorStream );
}
