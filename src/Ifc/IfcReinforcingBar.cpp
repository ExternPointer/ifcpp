...
void IFC4X3::IfcReinforcingBar::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcReinforcingBar::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcReinforcingBar::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	const size_t num_args = args.size();
	if( num_args != 14 ){ std::stringstream err; err << "Wrong parameter count for entity IfcReinforcingBar, expecting 14, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_NominalDiameter = IfcPositiveLengthMeasure::createObjectFromSTEP( args[9], map, errorStream );
	m_CrossSectionArea = IfcAreaMeasure::createObjectFromSTEP( args[10], map, errorStream );
	m_BarLength = IfcPositiveLengthMeasure::createObjectFromSTEP( args[11], map, errorStream );
	m_PredefinedType = IfcReinforcingBarTypeEnum::createObjectFromSTEP( args[12], map, errorStream );
