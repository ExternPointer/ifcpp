...
void IFC4X3::IfcRelAssignsToProduct::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcRelAssignsToProduct::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcRelAssignsToProduct::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 7 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelAssignsToProduct, expecting 7, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_RelatingProduct = IfcProductSelect::createObjectFromSTEP( args[6], map, errorStream );
}
