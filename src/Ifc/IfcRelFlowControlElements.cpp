...
IFC4X3::IfcRelFlowControlElements::IfcRelFlowControlElements( int tag ) { m_tag = tag; }
void IFC4X3::IfcRelFlowControlElements::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_tag << "= IFCRELFLOWCONTROLELEMENTS" << "(";
	...
	stream << ",";
	if( m_RelatingFlowElement ) { stream << "#" << m_RelatingFlowElement->m_tag; } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcRelFlowControlElements::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcRelFlowControlElements::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	const size_t num_args = args.size();
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelFlowControlElements, expecting 6, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_GlobalId = IfcGloballyUniqueId::createObjectFromSTEP( args[0], map, errorStream );
	readEntityReference( args[1], m_OwnerHistory, map, errorStream );
	m_Name = IfcLabel::createObjectFromSTEP( args[2], map, errorStream );
	m_Description = IfcText::createObjectFromSTEP( args[3], map, errorStream );
	readEntityReferenceList( args[4], m_RelatedControlElements, map, errorStream );
	readEntityReference( args[5], m_RelatingFlowElement, map, errorStream );
}
...
void IFC4X3::IfcRelFlowControlElements::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
...
void IFC4X3::IfcRelFlowControlElements::unlinkFromInverseCounterparts()
{
