...
IFC4X3::IfcRelVoidsElement::IfcRelVoidsElement( int tag ) { m_tag = tag; }
void IFC4X3::IfcRelVoidsElement::getStepLine( std::stringstream& stream ) const
{
	stream << "#" << m_tag << "= IFCRELVOIDSELEMENT" << "(";
	...
	stream << ",";
	if( m_RelatedOpeningElement ) { stream << "#" << m_RelatedOpeningElement->m_tag; } else { stream << "$"; }
	stream << ");";
}
void IFC4X3::IfcRelVoidsElement::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcRelVoidsElement::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	const size_t num_args = args.size();
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRelVoidsElement, expecting 6, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_GlobalId = IfcGloballyUniqueId::createObjectFromSTEP( args[0], map, errorStream );
	readEntityReference( args[1], m_OwnerHistory, map, errorStream );
	m_Name = IfcLabel::createObjectFromSTEP( args[2], map, errorStream );
	m_Description = IfcText::createObjectFromSTEP( args[3], map, errorStream );
	readEntityReference( args[4], m_RelatingBuildingElement, map, errorStream );
	readEntityReference( args[5], m_RelatedOpeningElement, map, errorStream );
}
void IFC4X3::IfcRelVoidsElement::getAttributes( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes ) const
{
...
void IFC4X3::IfcRelVoidsElement::getAttributesInverse( std::vector<std::pair<std::string, shared_ptr<BuildingObject> > >& vec_attributes_inverse ) const
{
	IFC4X3::IfcRelDecomposes::getAttributesInverse( vec_attributes_inverse );
}
void IFC4X3::IfcRelVoidsElement::setInverseCounterparts( shared_ptr<BuildingEntity> ptr_self_entity )
{
...
void IFC4X3::IfcRelVoidsElement::unlinkFromInverseCounterparts()
{
