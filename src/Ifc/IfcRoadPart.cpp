...
void IFC4X3::IfcRoadPart::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcRoadPart::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcRoadPart::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcRoadPart, expecting 11, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcRoadPartTypeEnum::createObjectFromSTEP( args[10], map, errorStream );
}
