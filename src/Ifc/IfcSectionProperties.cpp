...
void IFC4X3::IfcSectionProperties::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcSectionProperties::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcSectionProperties::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 3 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSectionProperties, expecting 3, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_SectionType = IfcSectionTypeEnum::createObjectFromSTEP( args[0], map, errorStream );
	readEntityReference( args[1], m_StartProfile, map, errorStream );
	readEntityReference( args[2], m_EndProfile, map, errorStream );
}
