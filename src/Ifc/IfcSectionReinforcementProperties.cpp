...
void IFC4X3::IfcSectionReinforcementProperties::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 6 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSectionReinforcementProperties, expecting 6, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_LongitudinalStartPosition = IfcLengthMeasure::createObjectFromSTEP( args[0], map, errorStream );
	m_LongitudinalEndPosition = IfcLengthMeasure::createObjectFromSTEP( args[1], map, errorStream );
	m_TransversePosition = IfcLengthMeasure::createObjectFromSTEP( args[2], map, errorStream );
	m_ReinforcementRole = IfcReinforcingBarRoleEnum::createObjectFromSTEP( args[3], map, errorStream );
	readEntityReference( args[4], m_SectionDefinition, map, errorStream );
	readEntityReferenceList( args[5], m_CrossSectionReinforcementDefinitions, map, errorStream );
}
