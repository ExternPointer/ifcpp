...
void IFC4X3::IfcSectionedSolidHorizontal::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 3 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSectionedSolidHorizontal, expecting 3, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	readEntityReference( args[0], m_Directrix, map, errorStream );
	readEntityReferenceList( args[1], m_CrossSections, map, errorStream );
	readEntityReferenceList( args[2], m_CrossSectionPositions, map, errorStream );
}
