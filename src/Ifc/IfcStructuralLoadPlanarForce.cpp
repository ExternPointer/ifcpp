...
void IFC4X3::IfcStructuralLoadPlanarForce::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 4 ){ std::stringstream err; err << "Wrong parameter count for entity IfcStructuralLoadPlanarForce, expecting 4, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_Name = IfcLabel::createObjectFromSTEP( args[0], map, errorStream );
	m_PlanarForceX = IfcPlanarForceMeasure::createObjectFromSTEP( args[1], map, errorStream );
	m_PlanarForceY = IfcPlanarForceMeasure::createObjectFromSTEP( args[2], map, errorStream );
	m_PlanarForceZ = IfcPlanarForceMeasure::createObjectFromSTEP( args[3], map, errorStream );
}
