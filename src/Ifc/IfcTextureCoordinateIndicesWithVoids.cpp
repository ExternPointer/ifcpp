...
void IFC4X3::IfcTextureCoordinateIndicesWithVoids::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 3 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTextureCoordinateIndicesWithVoids, expecting 3, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	readTypeOfIntegerList( args[0], m_TexCoordIndex );
	readEntityReference( args[1], m_TexCoordsOf, map, errorStream );
	readTypeOfIntegerList2D( args[2], m_InnerTexCoordIndices );
}
