...
void IFC4X3::IfcThirdOrderPolynomialSpiral::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcThirdOrderPolynomialSpiral::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 5 ){ std::stringstream err; err << "Wrong parameter count for entity IfcThirdOrderPolynomialSpiral, expecting 5, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	m_Position = IfcAxis2Placement::createObjectFromSTEP( args[0], map, errorStream );
	m_CubicTerm = IfcLengthMeasure::createObjectFromSTEP( args[1], map, errorStream );
	m_QuadraticTerm = IfcLengthMeasure::createObjectFromSTEP( args[2], map, errorStream );
	m_LinearTerm = IfcLengthMeasure::createObjectFromSTEP( args[3], map, errorStream );
	m_ConstantTerm = IfcLengthMeasure::createObjectFromSTEP( args[4], map, errorStream );
}
