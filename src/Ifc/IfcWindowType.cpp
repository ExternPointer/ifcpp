...
void IFC4X3::IfcWindowType::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcWindowType::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcWindowType::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 13 ){ std::stringstream err; err << "Wrong parameter count for entity IfcWindowType, expecting 13, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcWindowTypeEnum::createObjectFromSTEP( args[9], map, errorStream );
	m_PartitioningType = IfcWindowTypePartitioningEnum::createObjectFromSTEP( args[10], map, errorStream );
