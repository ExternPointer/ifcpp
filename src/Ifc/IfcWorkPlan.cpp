...
void IFC4X3::IfcWorkPlan::getStepLine( std::stringstream& stream ) const
{
...
void IFC4X3::IfcWorkPlan::getStepParameter( std::stringstream& stream, bool /*is_select_type*/ ) const { stream << "#" << m_tag; }
void IFC4X3::IfcWorkPlan::readStepArguments( const std::vector<std::string>& args, const std::map<int,shared_ptr<BuildingEntity> >& map, std::stringstream& errorStream )
{
	...
	if( num_args != 14 ){ std::stringstream err; err << "Wrong parameter count for entity IfcWorkPlan, expecting 14, having " << num_args << ". Entity ID: " << m_tag << std::endl; throw BuildingException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcWorkPlanTypeEnum::createObjectFromSTEP( args[13], map, errorStream );
}
